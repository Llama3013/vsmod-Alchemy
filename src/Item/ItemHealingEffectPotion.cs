...
namespace Alchemy
{
    public class ItemHealingEffectPotion : Item
    {
        ...
        public override void OnHeldInteractStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handling)
        {
            ...
            if (byEntity.WatchedAttributes.GetLong("healingeffectpotionid") == 0)
            {
                ...
                if (attr != null && attr["healingeffect"].Exists && attr["duration"].Exists)
                {
        ...
        public override void OnHeldInteractStop(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel)
        {
            if (secondsUsed > 1.45f && byEntity.World.Side == EnumAppSide.Server)
            {
                ...
                JsonObject attr = slot.Itemstack.Collectible.Attributes;
                float healingEffect = attr["healingeffect"].AsFloat();
                ...
                potionEntity.WatchedAttributes.SetLong("healingeffectpotionid", potionListenerId);

                /*This adds the attribute amount to the player's stats*/
                potionEntity.Stats.Set("healingeffectivness", "potionmod", healingEffect, false);
                ...
                if (potionEntity is EntityPlayer)
                {
                    ...
                    sPlayer.SendMessage(GlobalConstants.InfoLogChatGroup, "You feel the effects of the healing effectiveness potion.", EnumChatType.Notification);
                }
            }
        }
        ...
        {
            /*These two lines reset the player back to what they were before the potion*/
            potionEntity.Stats.Set("healingeffectivness", "potionmod", 0, false);
            potionEntity.WatchedAttributes.SetLong("healingeffectpotionid", 0);

            if (potionEntity is EntityPlayer)
            {
                IServerPlayer player = (potionEntity.World.PlayerByUid((potionEntity as EntityPlayer).PlayerUID) as IServerPlayer);
                player.SendMessage(GlobalConstants.InfoLogChatGroup, "You feel the effects of the healing effectiveness potion dissipate.", EnumChatType.Notification);
            }
        }
        ...
        public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
        {
            ...
            if (attr != null && attr["healingeffect"].Exists && attr["duration"].Exists)
            {
                float healingEffect = attr["healingeffect"].AsFloat();
                float duration = attr["duration"].AsFloat();
                dsc.AppendLine(Lang.Get("When used: +{0}% healing effectiveness. Lasts for {1} seconds.", healingEffect * 100, duration));
            }
        }
