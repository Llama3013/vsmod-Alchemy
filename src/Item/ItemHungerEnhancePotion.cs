...
namespace Alchemy
{
    public class ItemHungerEnhancePotion : Item
    {
        ...
        public override void OnHeldInteractStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handling)
        {
            ...
            if (byEntity.WatchedAttributes.GetLong("hungerenhancepotionid") == 0)
            {
                ...
                if (attr != null && attr["hungerenhance"].Exists && attr["duration"].Exists)
                {
        ...
        public override void OnHeldInteractStop(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel)
        {
            if (secondsUsed > 1.45f && byEntity.World.Side == EnumAppSide.Server)
            {
                ...
                JsonObject attr = slot.Itemstack.Collectible.Attributes;
                float hungerEnhance = attr["hungerenhance"].AsFloat();
                ...
                potionEntity.WatchedAttributes.SetLong("hungerenhancepotionid", potionListenerId);
                ...
                if (potionEntity is EntityPlayer)
                {
                    ...
                    if (potionEntity.WatchedAttributes.GetLong("hungersupresspotionid") != 0)
                    {
        ...
        private void onPotionCall(float dt)
        {
            ...
            potionEntity.Stats.Set("hungerrate", "potionmod", 0, false);
            potionEntity.WatchedAttributes.SetLong("hungerenhancepotionid", 0);

            if (potionEntity is EntityPlayer)
            {
                IServerPlayer player = (potionEntity.World.PlayerByUid((potionEntity as EntityPlayer).PlayerUID) as IServerPlayer);
                player.SendMessage(GlobalConstants.InfoLogChatGroup, "You feel the effects of the hunger enhancer potion dissipate.", EnumChatType.Notification);
            }
        }
        ...
        public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
        {
            ...
            if (attr != null && attr["hungerenhance"].Exists && attr["duration"].Exists)
            {
                float hungerEnhance = attr["hungerenhance"].AsFloat();
                float duration = attr["duration"].AsFloat();
                dsc.AppendLine(Lang.Get("When used: +{0}% hunger rate. Lasts for {1} seconds.", hungerEnhance * 100, duration));
            }
        }
