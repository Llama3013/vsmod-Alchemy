...
namespace Alchemy
{
    public class ItemHungerSupressPotion : Item
    {
        ...
        public override void OnHeldInteractStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handling)
        {
            ...
            if (byEntity.Stats.GetBlended("hungersupresspotionid") == 1)
            {
                ...
                if (attr != null && attr["hungersupress"].Exists && attr["duration"].Exists)
                {
            ...
            if (secondsUsed > 1.45f && byEntity.World.Side == EnumAppSide.Server)
            {
                ...
                if (potionEntity is EntityPlayer)
                {
                    ...
                    if (potionEntity.Stats.GetBlended("hungerenhancepotionid") != 1)
                    {
                        ...
                        player.SendMessage(GlobalConstants.InfoLogChatGroup, "You feel the effects of the hunger enhancer and hunger supressor potions combine and fizzle out.", EnumChatType.Notification);
                    }
                    else
                    {
                        player.SendMessage(GlobalConstants.InfoLogChatGroup, "You feel the effects of the hunger supress potion.", EnumChatType.Notification);
                        /*This adds the attribute amount to the player's stats*/
                        potionEntity.Stats.Set("hungerrate", "potionmod", hungerSupress, false);
                    }
                }
        ...
        private void onPotionCall(float dt)
        {
            ...
            {
                IServerPlayer player = (potionEntity.World.PlayerByUid((potionEntity as EntityPlayer).PlayerUID) as IServerPlayer);
                player.SendMessage(GlobalConstants.InfoLogChatGroup, "You feel the effects of the hunger supressser potion dissipate.", EnumChatType.Notification);
            }
        }
